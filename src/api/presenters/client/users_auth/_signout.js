...
export default async (data) => {
  ...
  // Check existing user
  let resultFindUser = await servFindOneUser(data.email, 'email', ['password', 'tokenResetPassword', 'tokenSignupConfirmation', 'deleted'], false)
  ...
  if (!resultCreatePassword.success || !resultCreatePassword.data) { return (httpMsg.http422('Error to generate user password.', errCode)) }
